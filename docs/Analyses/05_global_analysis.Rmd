---
title: Metadata and effective population size analysis
output:
    html_document:
        toc: true
        toc_float: true
...
knitr::opts_chunk$set(echo = T, warning=FALSE, collapse=T, cache=T, 'styler', tidy.opts=list(strict=T))
...
#Didnt work as intended becasue variables were saved with the name variable and loaded into that same name - find a fix in the future. 
# name_data = function(name){
#   paste0(path_data,name)
# }
# #Defining save function
# #Defaults to variable_name.Rdata
# save_data = function(variable) {
#   
#   save(eval(parse(text = model_name)), file=name_data(paste0(deparse(substitute(variable)),".Rdata")))
# }
# 
# load_data = function(variable) {
#   print(paste0(deparse(substitute(variable)),".Rdata"))
#   load(file=name_data(paste0(deparse(substitute(variable)),".Rdata")),envir =.GlobalEnv)
# }
# save_data(variable)
...
Here we describe large clades that differ in some metadata with respect to the rest of the tree clades of the tree. 
Specifically we investigate clades that differ wrt. the variants of the mosaic variants of the genes mtr and penA, minimum inhibitory concentration (MIC) (categorized into levels high and low) levels,  and  sex  distribution, 
...
p3=  p1+geom_facet(panel = "mtrMosaic", data =df_bar, geom = geom_col, 
  ...
  geom_facet(
      ...
      data = df_bar,
      geom = geom_col,
      aes(x = dummy_bar_value, fill = MLST),
      orientation = 'y',
      width = 1
    ) +
    geom_facet(
      ...
      data = df_bar,
      geom = geom_col,
      aes(x = dummy_bar_value, fill = locations),
      orientation = 'y',
      width = 1
    )+
  theme_tree2()+scale_fill_manual(breaks=c("non-mosaic","semi-mosaic","high","low","mosaic","mosaic1","mosaic2","mosaic3","mosaic4","F","M","nd", unique(metaDataTimeTree$locations), unique(metaDataTimeTree$MLST)),values=c(cols,"#A6CEE3","#1F78B4","#B2DF8A","#33A02C","#FB9A99", "#E31A1C","#FFFF99","#FFFFFF",'#e6194b', '#3cb44b', '#ffe119', '#4363d8', '#f58231', '#911eb4', '#46f0f0', '#f032e6', '#bcf60c', '#fabebe', '#008080', '#e6beff', '#9a6324', '#fffac8', '#800000', '#aaffc3', '#808000', '#ffd8b1', '#000075', '#808080', '#ffffff', '#000000',"#FF7F00"))#scale_fill_brewer(palette="Paired")
...
The annotated tree shows that most of the mosaic variant of mtr cluster in distict parts of the tree. 
Variants of penA that are related to high MIC also make up distinct clusters. 
The distribution of sex (M/F) does not display any obvious large scale patterns, but when zooming in on smaller cluseters (< 50 isolates) there are many clades that display higher fractions of females F/(F+M).
...
p2 = annotate(tree,extra_dat = metaDataTimeTree,size=0.2)
p2
...
#save(skyMtr,file = paste0(path_data, "skyMtr.Rdata"))
#load(file=paste0(path_data, "skyMtr.Rdata"))
skyhighMic_mosaic = skygrowth.map(penA_highMic_mosaic)
#save(skyhighMic_mosaic,file = paste0(path_data, "skyhighMic_mosaic.Rdata"))
#load(file=paste0(path_data, "skyhighMic_mosaic.Rdata"))
skyhighMic_mosaic2 = skygrowth.map(penA_highMic_mosaic2)
#save(skyhighMic_mosaic2, file = paste0(path_data, "skyhighMic_mosaic2.Rdata"))
#load(file=paste0(path_data, "skyhighMic_mosaic2.Rdata"))
skyhighMic_nonMosaic = skygrowth.map(penA_highMic_nonMosaic)
#save(skyhighMic_nonMosaic,file = paste0(path_data, "skyhighMic_nonMosaic.Rdata"))
#load(file=paste0(path_data, "skyhighMic_nonMosaic.Rdata"))
skyFullTree = skygrowth.map(tree)
#skyFullTree = skygrowth.map(tree, res=100)
#save(skyFullTree, file = paste0(path_data, "skyFullTree.Rdata"))
#load(file = paste0(path_data, "skyFullTree.Rdata"))
```
...
subtree_plots1(color1="#E69F00", color2="#56B4E9", mtr_clade = mtr_clade,penA_highMic_mosaic = penA_highMic_mosaic, skyMtr = skyMtr, skyhighMic_mosaic =  skyhighMic_mosaic)
...
```{r, echo=F, include=T, fig.height=10, fig.width=10, warning=F}
subtree_plots2(color1="#009E73", color2="#CC79A7",penA_highMic_mosaic2 = penA_highMic_mosaic2,penA_highMic_nonMosaic = penA_highMic_nonMosaic,skyhighMic_mosaic2 = skyhighMic_mosaic2,skyhighMic_nonMosaic = skyhighMic_nonMosaic)
...
#g6 = ggtree(tree,mrsd="2019-08-01")+theme_tree2()+scale_x_continuous(breaks=seq(1750,2020,20),expand=c(0,0))+theme_bw()+ylab("Tree position")+theme(axis.ticks.y = element_blank())+geom_highlight(node=9778,fill="green")
g6 = groupClade(tree, c(9778,13077,12872,12427))
color_mapping_tree = c("0"="#000000","1"="#E69F00" , "2"="#56B4E9", "3" = "#009E73" , "4" = "#CC79A7")
g6 = ggtree(g6, mrsd = "2019-08-01",aes(color=group))+scale_x_continuous(breaks=seq(1750,2020,20),expand=c(0,0))+scale_color_manual("Clade",values=color_mapping_tree)+theme(legend.position="none")
