...
```{r, include=TRUE}
#c1 = LineageHomology::Summarize_import_local_transmission(replicates_NOR); c1
#c2 = LineageHomology::Summarize_import_local_transmission(replicates_AUS); c2
#c3 = LineageHomology::Summarize_import_local_transmission(replicates_USA); c3
#c3 = LineageHomology::Summarize_import_local_transmission(replicates_Europe); c3

```


In Norway we estimate that there is 29% importation. This is more than twice the estimated amount in USA and Australia (each around 13%)
...
boxplot_boxplot_frame_import_by_LC = function(boxplot_frame, x_lab="", ylims=c(0,0.35)) {
  library(ggplot2)
  ggplot(boxplot_frame)+geom_boxplot(aes(x=Period,y=Import/(Import+LC),color=Location))+theme_bw()+
    ylab("Counts")+xlab(x_lab)+
    ggtitle("Import / ( Import + local transmission )")+
    theme(plot.title = element_text(hjust = 0.5))+
...
boxplot_boxplot_frame_Export_by_LC = function(boxplot_frame, x_lab="", ylims=c(0,0.35)) {
  ...
  ggplot(boxplot_frame)+geom_boxplot(aes(x=Period,y=Export/(Export+LC),color=Location))+theme_bw()+
    ylab("Counts")+xlab(x_lab)+
    ggtitle("Export / ( Export + local transmission )")+
    theme(plot.title = element_text(hjust = 0.5))+
    scale_color_manual(values = c("#1F85DE", "#DE781F"))+scale_y_continuous(expand = c(0, 0), limits = ylims)
...
boxplot_boxplot_frame_import_by_LC(comparison_NOR_AUS)
dev.off()
