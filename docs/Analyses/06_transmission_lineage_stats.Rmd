...
## Overview
In this summary, we provide an overview of some of the important statistics for the transmission lineages in different locations. It is evident from the [transmission lineage explorer](https://magnunos.shinyapps.io/LineageHomology_Explorer/) that some transmission lineages account for a disproportionate amount of the sequences observed in Europe, the USA, Norway, and Australia. However, most transmission lineages in all locations are small. We compare the different locations in terms of transmission lineage size distributions and consider the following research questions (RQs):

1. What percentage of local cases are explained by the 10 largest lineages, and what percentage are explained by singletons?
2. How are the lineage sizes distributed? Do they follow a power law?

> It should be kept in mind that transmission lineages with similar sizes may have grown in different ways. For example, a slow-spreading, old lineage may have a similar size to a fast-spreading, young lineage.
...
```
 ...
 To fit a power law distribution, we use the fit_power_law function from the `igraph` `R-package`. We then use the Kolmogorov-Smirnov test included in the package to calculate the p-value, which indicates whether the data is significantly different from what we expect under a power law distribution with the estimated coefficient.
...
```{r remedy001}
# Code sequences to extract some info for the paper - can be deleted.
# inds = unlist(lapply(eur_tips, FUN = function(x) which(x==name_date$name)))
# 
# nor_tips %in% eur_tips
# remove_norwegian = unlist(lapply(nor_tips, FUN = function(x) which(x==eur_tips)))
# eur_without_nor = eur_tips[-remove_norwegian]
# 
# inds = unlist(lapply(eur_without_nor, FUN = function(x) which(x==name_date$name)))
# length(name_date$dates[inds][name_date$dates[inds] >= 2013 & name_date$dates[inds] <= 2018])
